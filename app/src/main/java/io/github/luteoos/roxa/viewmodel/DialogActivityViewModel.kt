...
import io.github.luteoos.roxa.utils.MultipartBodyFiller
...
class DialogActivityViewModel : BaseViewModel() {
    ...
    fun addEvent(name: String, desc: String?, localization: String?){
        if(start == "" || end == "" || name.isEmpty())
            ...
            send(Parameters.SHOW_PROGRESS_BAR)
            Rest.createServiceMultipart(BasicApi::class.java, Session.token).let { client ->
                disposable.add(client.createEvent(Session.userUUIDString, teamId, MultipartBodyFiller.getMultipartEventBody(CreateEventRequest(
